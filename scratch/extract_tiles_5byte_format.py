"""Extract all tiles using the discovered 5-byte format.

Format: [flags/type][offset_le16][width_4px][height_4px]
Records appear to be in NULL-separated sequences.
"""
...
def extract_tiles_5byte():
    """Extract tiles using 5-byte format within NULL-separated sequences."""
    print("Extracting Tiles Using 5-Byte Format")
    print("=" * 70)
    ...
    # Parse each sequence as 5-byte records
    tiles = []
    tile_idx = 0

    for seq_idx, (seq_start, seq_data) in enumerate(sequences):
        pos = 0
        while pos + 5 <= len(seq_data):
            flags = seq_data[pos]
            offset_le16 = seq_data[pos + 1] | (seq_data[pos + 2] << 8)
            width_4px = seq_data[pos + 3]
            height_4px = seq_data[pos + 4]
            ...
            if 4 <= width_px <= 256 and 4 <= height_px <= 256:
                ...
                if offset_le16 + required_bytes <= len(tile_data_region):
                    ...
                    if tile_idx < 30:
                        print(f"  Tile {tile_idx:3d} @ meta 0x{seq_start + pos:04X}: flags=0x{flags:02X}, offset=0x{offset_le16:04X}, dims={width_px:3d}x{height_px:3d}")
    ...
    # Extract tiles
    output_dir = Path("tiles_5byte_format")
    output_dir.mkdir(exist_ok=True)

    print(f"Extracting all tiles to {output_dir}/...")
    print("-" * 70)
    ...
    for tile in tiles:
        try:
            ...
            filename = f"tile_{tile['index']:03d}_flags{flags:02X}_{width}x{height}.png"
            img.save(output_dir / filename)

            # Save 4x
            img_4x = img.resize((width * 4, height * 4), Image.NEAREST)
            filename_4x = f"tile_{tile['index']:03d}_flags{flags:02X}_{width}x{height}_4x.png"
            img_4x.save(output_dir / filename_4x)

            unique_colors = len(set(sprite.pixels))

            if tile['index'] < 10 or tile['index'] % 20 == 0:
                print(f"  Tile {tile['index']:3d}: {width:3d}x{height:3d}, {unique_colors:2d} colors, flags=0x{flags:02X}")
    ...
    print()
    print(f"Check tile_028_flagsC9_32x32_4x.png - this should be the correct one!")
