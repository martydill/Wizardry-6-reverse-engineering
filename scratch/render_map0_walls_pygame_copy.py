...
def decode_wall_planes(data: bytes, base: int):
    ...
    for block in range(12):
        for row in range(8):
            for col in range(8):
                # Transpose B10 and B11 (stored in column-major order)
                if block >= 10:
                    idx = (block << 6) + (col << 3) + row
                else:
...
def draw_block_quad_2x2(screen, ox, oy, cell, fields, color, width=2, extra_blocks=(10, 11)):
    ...
    e0, e1 = extra_blocks
    layout = {2: (0, 0), 3: (1, 0), 0: (0, 1), 1: (1, 1), e0: (0, 2), e1: (1, 2)}
...
def build_mode_boundaries(a_vals, b_vals, origins, block, include_cross_boundaries=True):
    """Build canonical 9x9 boundary arrays for one block."""
    ...
    for y in range(9):
        for x in range(8):
            if y == 0:
                if include_cross_boundaries:
                    h[y][x] = wall_mode_value(a_vals, b_vals, origins, block, 0, x, 2) != 0
                else:
                    h[y][x] = False
            else:
                h[y][x] = wall_mode_value(a_vals, b_vals, origins, block, y - 1, x, 0) != 0

    for y in range(8):
        for x in range(9):
            if x == 0:
                if include_cross_boundaries:
                    v[y][x] = wall_mode_value(a_vals, b_vals, origins, block, y, 0, 3) != 0
                else:
                    v[y][x] = False
            else:
                v[y][x] = wall_mode_value(a_vals, b_vals, origins, block, y, x - 1, 1) != 0
    return h, v


def draw_block_quad_resolved(
    screen, ox, oy, cell, fields, origins, color, width=2, extra_blocks=(10, 11)
):
    ...
    for block, (gx, gy) in layout.items():
        px = ox + gx * (panel_w + gap)
        py = oy + gy * (panel_h + gap)
        # Local cell-edge reconstruction only (ignore cross-block boundaries).
        # This avoids seam artifacts and row-phase drift while we focus on
        # per-block correctness for b0/b1/b2/b3/b10/b11.
        for row in range(8):
            dr = 7 - row
            for col in range(8):
                s = a_vals[block][row][col] != 0
                e = b_vals[block][row][col] != 0
                n = a_vals[block][row - 1][col] != 0 if row > 0 else False
                w = b_vals[block][row][col - 1] != 0 if col > 0 else False
                ...
                if s:
                    # +Y (south) edge belongs to the bottom boundary of the cell.
                    pygame.draw.line(screen, color, (x0, y1), (x1, y1), width)
                if n:
                    pygame.draw.line(screen, color, (x0, y0), (x1, y0), width)
                if w:
...
def main():
    ...
    # Choose bottom-row inspection pair from actual modified-vs-original deltas.
    delta_counts = {}
    o = {(t["block"], t["row"], t["col"]): (t["south"], t["east"]) for t in orig_planes}
    for t in mod_planes:
        k = (t["block"], t["row"], t["col"])
        if o.get(k) != (t["south"], t["east"]):
            delta_counts[t["block"]] = delta_counts.get(t["block"], 0) + 1
    candidates = [b for b, _ in sorted(delta_counts.items(), key=lambda kv: (-kv[1], kv[0])) if b not in {0, 1, 2, 3}]
    while len(candidates) < 2:
        for fb in (10, 11, 9, 8, 7, 6, 5, 4):
            if fb not in candidates and fb not in {0, 1, 2, 3}:
                candidates.append(fb)
                if len(candidates) == 2:
                    break
    extra_blocks = (candidates[0], candidates[1])
    print(f"[debug] delta block counts: {delta_counts}")
    print(f"[debug] inspection extra blocks: {extra_blocks}")
    if args.stitched:
        ...
        # Local block rendering path: ignore layout/stitching entirely.
        fixed = {}
        layout = [(0, 0)] * 12
        orig_stitched = {}
        mod_stitched = {}
    ...
    # Build static frame once (content does not animate), then just blit.
    frame = pygame.Surface(screen.get_size()).convert()
    frame.fill((26, 26, 30))
    draw_grid(frame, ox, oy, cell)

    # Full-map baseline decode first.
    draw_baseline_walls(frame, ox, oy, cell, orig_baseline, (70, 100, 130), 1)
    draw_baseline_walls(frame, ox, oy, cell, mod_baseline, (170, 200, 230), 2)

    # Original walls (dim) then modified walls (bright), then added walls (red).
    draw_perimeter(frame, ox, oy, cell, orig_f, (100, 100, 130), 3)
    draw_perimeter(frame, ox, oy, cell, mod_f, (210, 210, 240), 4)
    draw_perimeter(frame, ox, oy, cell, add_f, (255, 80, 80), 7)

    # Full wall-plane atlas (all original and modified walls, not just edited subset).
    atlas_x = ox + 20 * cell + 70
    atlas_y = oy
    draw_block_atlas(frame, atlas_x, atlas_y, 10, orig_planes, (100, 100, 130), 1, flip_v=True)
    draw_block_atlas(frame, atlas_x, atlas_y, 10, mod_planes, (220, 220, 240), 2, flip_v=True)

    # Focused 2x2 inspection panel: B2 B3 over B0 B1.
    quad_x = atlas_x
    quad_y = atlas_y + 3 * (8 * 10 + 10) + 28
    draw_block_quad_2x2(
        frame, quad_x, quad_y, 18, orig_planes, (100, 100, 130), 1, extra_blocks=extra_blocks
    )
    draw_block_quad_2x2(
        frame, quad_x, quad_y, 18, mod_planes, (220, 220, 240), 2, extra_blocks=extra_blocks
    )
    ...
    if args.stitched:
        ...
        draw_stitched(frame, stitched_x, stitched_y, 24, orig_stitched, (110, 110, 145), 1, bounds=bounds)
        draw_stitched(frame, stitched_x, stitched_y, 24, mod_stitched, (220, 220, 245), 2, bounds=bounds)
        draw_block_origin_overlay(frame, stitched_x, stitched_y, 24, bounds, layout)
    ...
    lines = [
        ...
        f"Bottom-right panel: B2 B3 over B0 B1 over B{extra_blocks[0]} B{extra_blocks[1]} (local-cell edges)",
        "Use --stitched to show experimental global composite",
        f"Block local rotation: {args.block_rot} deg",
        f"Auto layout (true-edge overlap score): {layout if args.stitched else 'disabled (local-only mode)'}",
        f"Fixed block anchors: {fixed if args.stitched else 'disabled (local-only mode)'}",
        "Blue dim=original, white=modified, red=added",
        "Esc = quit",
    ]
    for i, text in enumerate(lines):
        surf = font.render(text, True, (220, 220, 220))
        frame.blit(surf, (80, 20 + i * 22))
